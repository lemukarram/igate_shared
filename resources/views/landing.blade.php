...
<body class="bg-gray-50 text-gray-800 overflow-x-hidden selection:bg-[#3da9e4] selection:text-white">
    ...
    <nav class="fixed w-full z-50 glass-card">
        <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            ...
            <div class="hidden lg:flex items-center space-x-10 text-sm font-medium text-gray-600 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
            ...
            <div class="flex items-center space-x-4 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
                <button onclick="toggleLang()" class="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-gray-100 transition-all">
                    <i data-lucide="languages" class="w-4 h-4 text-gray-500"></i>
                    <span class="text-xs font-bold uppercase text-gray-700">{{ app()->getLocale() === 'en' ? 'العربية' : 'English' }}</span>
                </button>
                <a href="/login" class="text-sm font-medium text-gray-700 hover:theme-text transition-colors">{{ __('common.signin') }}</a>
                <a href="/login" class="theme-bg text-white px-6 py-2.5 rounded-lg font-medium text-sm theme-hover-bg transition-all shadow-md active:scale-95">{{ __('common.get_started') }}</a>
            </div>
        </div>
    </nav>
    ...
    <section class="relative pt-40 pb-20 px-6 hero-gradient overflow-hidden">
        <div class="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between relative z-10 gap-12">
            <div class="w-full lg:w-1/2 {{ app()->getLocale() == 'ar' ? 'text-right' : 'text-left' }}">
                <div class="inline-flex items-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} bg-white border border-[#3da9e4] theme-text px-4 py-1.5 rounded-md text-xs font-medium uppercase tracking-wider mb-6 shadow-sm">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-4">
                    <a href="/login" class="w-full sm:w-auto px-8 py-3.5 theme-bg text-white rounded-lg font-medium hover:bg-[#2b8bc2] transition-all shadow-lg flex items-center justify-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
            ...
            <div class="w-full lg:w-1/2 relative animate-float">
                <div class="glass-card rounded-lg p-3 shadow-xl relative z-10 border-t border-l border-white/60">
                    ...
                    <div class="absolute {{ app()->getLocale() == 'ar' ? '-right-6' : '-left-6' }} top-10 bg-white p-4 rounded-lg shadow-lg border border-gray-100 flex items-center space-x-3 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
                        ...
                        <div>
                            <p class="text-xs text-gray-400 font-medium uppercase tracking-wide">{{ __('common.payment_released') }}</p>
                            <p class="text-sm font-semibold">24,500 {{ __('common.sar') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="absolute {{ app()->getLocale() == 'ar' ? '-left-6' : '-right-6' }} bottom-10 bg-white p-4 rounded-lg shadow-lg border border-gray-100 flex items-center space-x-3 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }}">
                        ...
                        <div>
                            <p class="text-xs text-gray-400 font-medium uppercase tracking-wide">{{ __('common.sla_verified') }}</p>
                            <p class="text-sm font-semibold">{{ __('common.success_rate') }}</p>
                        </div>
                    </div>
                </div>
                <!-- decorative blobs -->
    ...
    <section id="why-us" class="py-24 bg-white relative">
        <div class="max-w-7xl mx-auto px-6">
            <div class="text-center mb-16">
                <h2 class="text-3xl lg:text-4xl font-semibold text-gray-900 mb-4">{{ __('common.why_igate_title') }}</h2>
                <p class="text-gray-500 font-medium max-w-2xl mx-auto">{{ __('common.why_igate_subtitle') }}</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div class="p-8 rounded-lg border border-gray-100 bg-gray-50 hover:bg-white hover:shadow-xl transition-all duration-300 group cursor-pointer {{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                    ...
                    <h3 class="text-xl font-semibold mb-3 text-gray-800">{{ __('common.escrow_security') }}</h3>
                ...
                <div class="p-8 rounded-lg border border-gray-100 bg-gray-50 hover:bg-white hover:shadow-xl transition-all duration-300 group cursor-pointer mt-0 lg:mt-8 {{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                    ...
                    <h3 class="text-xl font-semibold mb-3 text-gray-800">{{ __('common.fixed_scopes') }}</h3>
                ...
                <div class="p-8 rounded-lg border border-gray-100 bg-gray-50 hover:bg-white hover:shadow-xl transition-all duration-300 group cursor-pointer {{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                    ...
                    <h3 class="text-xl font-semibold mb-3 text-gray-800">{{ __('common.verified_providers') }}</h3>
                ...
                <div class="p-8 rounded-lg border border-gray-100 bg-gray-50 hover:bg-white hover:shadow-xl transition-all duration-300 group cursor-pointer mt-0 lg:mt-8 {{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                    ...
                    <h3 class="text-xl font-semibold mb-3 text-gray-800">{{ __('common.sla_tracking') }}</h3>
        ...
        <div class="max-w-7xl mx-auto">
            <div class="flex flex-col md:flex-row justify-between items-end mb-12 border-b border-gray-200 pb-6">
                <div class="max-w-2xl {{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                    <h2 class="text-3xl font-semibold text-gray-900 mb-3">{{ __('common.core_services_catalog') }}</h2>
                    <p class="text-gray-500 font-normal">{{ __('common.core_services_subtitle') }}</p>
                </div>
                <a href="/login" class="theme-text font-medium flex items-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} hover:text-[#2b8bc2] transition-colors mt-4 md:mt-0">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div class="bg-white p-8 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group {{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                    ...
                    <h3 class="text-xl font-semibold mb-3 text-gray-800">{{ __('common.zatca_vat') }}</h3>
                    <p class="text-sm text-gray-500 leading-relaxed font-normal mb-6">{{ __('common.zatca_vat_description') }}</p>
                    <a href="/login" class="text-sm theme-text font-medium flex items-center space-x-1 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} group-hover:underline">
                ...
                <div class="bg-white p-8 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group {{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                    ...
                    <h3 class="text-xl font-semibold mb-3 text-gray-800">{{ __('common.human_capital') }}</h3>
                ...
                <div class="bg-white p-8 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group {{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                    ...
                    <h3 class="text-xl font-semibold mb-3 text-gray-800">{{ __('common.legal_advisory') }}</h3>
    ...
    <footer class="bg-white border-t border-gray-200 py-16 px-6">
        <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
            ...
            <div class="{{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                <h4 class="font-semibold text-gray-800 mb-6 text-sm uppercase tracking-wide">{{ __('common.marketplace') }}</h4>
            ...
            <div class="{{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                <h4 class="font-semibold text-gray-800 mb-6 text-sm uppercase tracking-wide">{{ __('common.governance') }}</h4>
                <ul class="space-y-4 text-sm text-gray-500 font-medium">
            ...
            <div class="{{ app()->getLocale() == 'ar' ? 'text-right' : '' }}">
                <h4 class="font-semibold text-gray-800 mb-6 text-sm uppercase tracking-wide">{{ __('common.support') }}</h4>
                <ul class="space-y-4 text-sm text-gray-500 font-medium">
