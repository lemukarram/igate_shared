...
<div class="w-full max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700" 
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-4xl font-normal text-gray-900 tracking-tight" x-text="t('common.dashboard')"></h1>
            <p class="text-gray-500 font-normal mt-1 text-lg" x-text="t('common.agency_performance')"></p>
        </div>
        <div class="flex items-center gap-3">
            <button @click="statusModalOpen = true" 
                  ...
                  class="px-4 py-2 rounded-[0.5rem] text-sm font-normal uppercase tracking-widest border transition-all flex items-center gap-2">
        ...
        <div x-show="statusModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center" style="display: none;">
            ...
            <div class="bg-white w-full max-w-sm rounded-xl shadow-2xl relative z-10 p-6 border border-gray-100">
                ...
                <div class="space-y-2">
                    <button @click="updateStatus('active')" class="w-full flex items-center gap-3 p-3 border border-gray-100 rounded-lg cursor-pointer hover:bg-gray-50 text-green-600 font-normal" :class="status === 'active' ? 'bg-green-50 border-green-200' : ''">
                        <div class="w-2 h-2 rounded-full bg-green-500"></div>
                        <span x-text="t('common.active')"></span>
                    </button>
                    <button @click="updateStatus('inactive')" class="w-full flex items-center gap-3 p-3 border border-gray-100 rounded-lg cursor-pointer hover:bg-gray-50 text-gray-400 font-normal" :class="status === 'inactive' ? 'bg-gray-50 border-gray-200' : ''">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 rounded-[2.5rem] shadow-xl shadow-blue-100 text-white relative overflow-hidden group">
            <div class="relative z-10">
                <p class="text-[10px] font-normal uppercase tracking-[0.2em] opacity-80 mb-2" x-text="t('common.total_revenue')"></p>
                <h3 class="text-3xl font-normal mb-1 leading-none">{{ number_format($totalRevenue) }} <span class="text-sm" x-text="lang === 'ar' ? 'ر.س' : 'SAR'"></span></h3>
                <p class="text-xs font-normal text-blue-100">{{ $growth >= 0 ? '+' : '' }}{{ number_format($growth, 1) }}% this month</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-emerald-500 to-teal-700 p-8 rounded-[2.5rem] shadow-xl shadow-emerald-100 text-white relative overflow-hidden group">
            <div class="relative z-10">
                <p class="text-[10px] font-normal uppercase tracking-[0.2em] opacity-80 mb-2" x-text="t('common.active_clients')"></p>
                <h3 class="text-3xl font-normal mb-1 leading-none">{{ $activeClientsCount }}</h3>
                <p class="text-xs font-normal text-emerald-100">Across all active projects</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-rose-500 to-pink-700 p-8 rounded-[2.5rem] shadow-xl shadow-rose-100 text-white relative overflow-hidden group">
            <div class="relative z-10">
                <p class="text-[10px] font-normal uppercase tracking-[0.2em] opacity-80 mb-2" x-text="t('common.sla_compliance')"></p>
                <h3 class="text-3xl font-normal mb-1 leading-none">{{ number_format($slaCompliant, 1) }}%</h3>
                <p class="text-xs font-normal text-rose-100" x-text="t('common.target').replace(':target', '98.0%')"></p>
            </div>
        ...
        <div class="bg-gradient-to-br from-amber-500 to-orange-600 p-8 rounded-[2.5rem] shadow-xl shadow-amber-100 text-white relative overflow-hidden group">
            <div class="relative z-10">
                <p class="text-[10px] font-normal uppercase tracking-[0.2em] opacity-80 mb-2" x-text="t('common.pending_tasks')"></p>
                <h3 class="text-3xl font-normal mb-1 leading-none">{{ $pendingTasksCount }}</h3>
                <p class="text-xs font-normal text-amber-100">Tasks in To-Do or In-Progress</p>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-8">
            <div class="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm">
                <div class="flex items-center justify-between mb-10">
                    <h3 class="text-xl font-normal" x-text="t('common.revenue_growth')"></h3>
                    <select class="bg-gray-50 border-none rounded-xl text-xs font-normal py-2 px-4 focus:ring-0">
                ...
                <div class="h-80 w-full flex items-center justify-center bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-100">
                    <p class="text-gray-400 font-normal text-sm uppercase tracking-widest italic" x-text="t('common.revenue_data_placeholder')"></p>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm">
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-xl font-normal" x-text="t('common.ongoing_projects')"></h3>
                    <a href="{{ route('provider.clients') }}" class="text-xs font-normal text-primary uppercase tracking-widest hover:underline" x-text="t('common.view_all')"></a>
                </div>
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h4 class="text-sm font-normal text-gray-900">{{ $p->service->name }}</h4>
                                <p class="text-[10px] font-normal text-gray-400 uppercase tracking-widest">{{ $p->client->name }}</p>
                            </div>
                        </div>
        ...
        <div class="space-y-8">
            <div class="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm">
                <h3 class="text-xl font-normal mb-10" x-text="t('common.client_retention')"></h3>
                <div class="space-y-8">
                    <div class="space-y-2">
            ...
            <div class="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm">
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-lg font-normal" x-text="t('common.team')"></h3>
                </div>
                <div class="space-y-4">
                    <a href="{{ route('provider.team_tasks') }}" class="block p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-primary/20 hover:bg-primary-light transition-all group">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <h4 class="text-xs font-normal text-gray-900 truncate" x-text="t('common.team')"></h4>
                            </div>
                        </div>
                    </a>
                </div>
            ...
            <div class="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm">
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-lg font-normal" x-text="t('common.pre_sale_chats')"></h3>
                </div>
                <div class="space-y-4">
                    ...
                    <a href="{{ route('explore.chat', ['serviceId' => $chat->service_id, 'providerId' => $chat->provider_id, 'client_id' => $chat->client_id]) }}" class="block p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-primary/20 hover:bg-primary-light transition-all group">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <h4 class="text-xs font-normal text-gray-900 truncate">{{ $chat->service->name }}</h4>
                                <p class="text-[9px] font-normal text-gray-400 uppercase tracking-widest truncate">{{ $chat->client->name }}</p>
                            </div>
                        </div>
                    </a>
                    @empty
                    ...
                    <a href="{{ route('provider.pre_sale_chats.index') }}" class="sidebar-item {{ request()->routeIs('provider.pre_sale_chats.*') ? 'active' : '' }} flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-600 transition-all">
                        <span x-show="!sidebarCollapsed" class="text-sm font-normal whitespace-nowrap" x-text="t('common.view_all')"></span>
                    </a>
                </div>
            </div>
        </div>
