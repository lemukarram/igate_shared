...
<body class="bg-gray-50 flex flex-col items-center justify-center min-h-screen p-6">
    ...
    <div class="max-w-md w-full bg-white p-8 rounded-lg shadow-lg border border-gray-100">
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-semibold text-gray-900" id="form-title">{{ __('common.welcome_back') }}</h1>
        ...
        <form id="login-form" action="{{ route('login.post') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('common.email_address') }}</label>
            ...
            <div>
                <div class="flex justify-between items-center mb-1.5">
                    <label class="block text-sm font-medium text-gray-700">{{ __('common.password') }}</label>
                    <a href="{{ route('password.request') }}" class="text-sm theme-text font-medium hover:underline">{{ __('common.forgot_password') }}</a>
                </div>
            ...
            <div class="flex items-center">
                ...
                <label for="remember" class="ml-2 text-sm text-gray-600 font-medium">{{ __('common.remember_me') }}</label>
            </div>
        ...
        <form id="register-form" action="{{ route('register.post') }}" method="POST" class="space-y-4 hidden">
            @csrf
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('common.fullname_company') }}</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('common.phone_number') }}</label>
            ...
            <div class="pt-2">
                <div class="flex items-center mb-3 p-3 bg-[#f0f8fd] border border-[#3da9e4]/30 rounded-lg">
                    ...
                    <label for="join_as_provider" class="ml-2 text-sm text-gray-800 font-semibold cursor-pointer">{{ __('common.join_as_provider') }}</label>
                </div>
                <div class="flex items-start">
                    ...
                    <label for="agree_terms" class="ml-2 text-sm text-gray-600 font-medium">{{ __('common.agree_terms') }} <a href="/terms" class="theme-text hover:underline" target="_blank">{{ __('common.terms_conditions') }}</a></label>
                </div>
            </div>
        ...
        <div class="mt-8 pt-6 border-t border-gray-100 text-center text-sm text-gray-500 font-medium">
            <p id="toggle-text">
                {{ __('common.dont_have_account') }} <button type="button" onclick="toggleForms()" class="theme-text hover:underline ml-1 font-semibold">{{ __('common.signup') }}</button>
            </p>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleForms() {
            ...
            if (isLogin) {
                ...
                toggleText.innerHTML = `${dict.dont_have_account} <button type="button" onclick="toggleForms()" class="theme-text hover:underline ml-1 font-semibold">${dict.signup}</button>`;
            } else {
                loginForm.classList.add('hidden');
                registerForm.classList.remove('hidden');
                title.textContent = dict.create_account_title;
                subtitle.textContent = dict.join_marketplace_subtitle;
                toggleText.innerHTML = `${dict.already_have_account} <button type="button" onclick="toggleForms()" class="theme-text hover:underline ml-1 font-semibold">${dict.signin}</button>`;
            }
        }
