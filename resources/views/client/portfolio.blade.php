...
<div class="max-w-7xl w-full space-y-8 animate-in fade-in duration-700">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900" x-text="t('common.my_portfolio_companies')"></h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
            ...
            <p class="text-sm text-gray-400 font-medium mb-6" x-text="'{{ $company->industry }}' ? '{{ $company->industry }}' : t('common.general_business')"></p>
            
            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 mb-6">
                <div class="text-center">
                    <span class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1" x-text="t('common.projects')"></span>
                    <span class="text-lg font-bold text-gray-900">{{ $company->projects_count }}</span>
                </div>
                <div class="w-px h-8 bg-gray-200"></div>
                <div class="text-center">
                    <span class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1" x-text="t('common.status')"></span>
                    <span class="text-[10px] font-black uppercase px-2 py-0.5 rounded-md {{ $company->is_active ? 'text-green-500 bg-green-50' : 'text-red-500 bg-red-50' }}" x-text="{{ $company->is_active ? 'true' : 'false' }} ? t('common.active') : t('common.inactive')"></span>
                </div>
            </div>
        ...
        <div class="col-span-full py-24 text-center bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
            ...
            <h3 class="text-2xl font-bold text-gray-900" x-text="t('common.start_adding_company')"></h3>
...
<div id="add-company-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center hidden">
    <div class="bg-white rounded-[2.5rem] p-10 max-w-md w-full mx-4 shadow-2xl animate-in zoom-in duration-300">
        <div class="flex items-center justify-between mb-8">
            <h3 class="text-2xl font-bold text-gray-900" x-text="t('common.add_new_company')"></h3>
