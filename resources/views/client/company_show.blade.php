...
<div class="max-w-7xl w-full space-y-8 animate-in fade-in duration-700">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 font-medium" x-text="t('common.manage_company_subtitle')"></p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-bold text-gray-900" x-text="t('common.projects')"></h2>
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div class="py-12 text-center bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                        <p class="text-gray-500 font-medium" x-text="t('common.no_projects_yet')"></p>
                    </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-bold text-gray-900" x-text="t('common.pre_sale_chats')"></h2>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div class="bg-blue-50/30 border border-blue-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 font-medium">{{ $chat->provider->providerProfile->company_name }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-4">
                            <span class="text-[10px] font-black uppercase px-2 py-0.5 rounded-md text-blue-500 bg-blue-100/50" x-text="t('common.consultation')"></span>
                    ...
                    @empty
                    <div class="py-12 text-center bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                        <p class="text-gray-500 font-medium" x-text="t('common.no_pre_sale_chats_yet')"></p>
                    </div>
                    @endforelse
                </div>
            </div>
        ...
        <div class="space-y-6">
            <h2 class="text-xl font-bold text-gray-900" x-text="t('common.company_profile')"></h2>
            <div class="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
                ...
                <div class="space-y-6">
                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1" x-text="t('common.industry')"></p>
                        <p class="text-sm font-bold text-gray-900">{{ $company->industry ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1" x-text="t('common.registration_number')"></p>
                        <p class="text-sm font-bold text-gray-900">{{ $company->registration_number ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1" x-text="t('common.about')"></p>
                        <p class="text-sm font-medium text-gray-600 leading-relaxed">{{ $company->about ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1" x-text="t('common.documents')"></p>
                        @php
                        ...
                        <div class="space-y-2 mt-2">
                            @forelse($documents as $doc)
                                <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank" class="flex items-center p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors group">
                                    ...
                                    <span class="text-xs font-bold text-gray-700 truncate flex-1">{{ $doc->name }}</span>
                ...
                <div class="mt-8 pt-8 border-t border-gray-50">
                    <button onclick="document.getElementById('edit-company-modal').classList.remove('hidden')" class="w-full py-3 bg-gray-900 text-white rounded-xl text-sm font-bold hover:bg-black transition-all flex items-center justify-center gap-2 shadow-lg shadow-gray-200">
...
<div id="edit-company-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center hidden">
    <div class="bg-white rounded-[2.5rem] p-10 max-w-xl w-full mx-4 shadow-2xl animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar">
        <div class="flex items-center justify-between mb-8">
            <h3 class="text-2xl font-bold text-gray-900" x-text="t('common.edit_company')"></h3>
        ...
        <form action="{{ route('companies.update', $company->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1">
                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-400" x-text="t('common.company_name')"></label>
                    <input type="text" name="name" value="{{ $company->name }}" required class="w-full px-5 py-3 border border-gray-100 bg-gray-50 rounded-2xl outline-none focus:ring-4 focus:ring-primary/10 transition-all font-medium text-sm">
                </div>
                <div class="space-y-1">
                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-400" x-text="t('common.industry')"></label>
                    <input type="text" name="industry" value="{{ $company->industry }}" class="w-full px-5 py-3 border border-gray-100 bg-gray-50 rounded-2xl outline-none focus:ring-4 focus:ring-primary/10 transition-all font-medium text-sm">
                </div>
            </div>

            <div class="space-y-1">
                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400" x-text="t('common.registration_number')"></label>
                <input type="text" name="registration_number" value="{{ $company->registration_number }}" class="w-full px-5 py-3 border border-gray-100 bg-gray-50 rounded-2xl outline-none focus:ring-4 focus:ring-primary/10 transition-all font-medium text-sm">
            </div>

            <div class="space-y-1">
                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400" x-text="t('common.about')"></label>
                <textarea name="about" rows="4" class="w-full px-5 py-4 border border-gray-100 bg-gray-50 rounded-2xl outline-none focus:ring-4 focus:ring-primary/10 transition-all font-medium text-sm resize-none">{{ $company->about }}</textarea>
            </div>

            <div class="space-y-1">
                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400" x-text="t('common.upload_documents')"></label>
                <input type="file" name="documents[]" multiple class="w-full px-5 py-3 border border-gray-100 bg-gray-50 rounded-2xl outline-none focus:ring-4 focus:ring-primary/10 transition-all font-medium text-sm">
                <p class="text-[10px] text-gray-400 mt-1" x-text="t('common.upload_documents_hint')"></p>
            </div>

            <button type="submit" class="w-full py-4 bg-primary text-white rounded-2xl font-bold text-sm hover:bg-primary-dark transition-all shadow-xl shadow-primary/20">
